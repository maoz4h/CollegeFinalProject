...
public class Row {
    ...
    SongData ParseRowToData()
    {
        SongData songData = new SongData();
        Vector<Double> HighLevelData = new Vector<Double>();
        
        for (int i = 2; i < row.size(); i++)
        {
            HighLevelData.add(Double.parseDouble(row.get(i).getKey().toString()));
        }
        songData.setHighLevelVector(HighLevelData);
        songData.setHasData(true);
        return songData;
    }
}
